...
contract StaticMarket {
		...
		returns (uint)
	{
		...
		) = abi.decode(extra, (address[2], uint256[3], string, bytes));
		...
		require(new_fill <= uints[1],"anyERC1155ForERC20: new fill exceeds maximum fill");
		require(SafeMath.mul(tokenIdAndNumeratorDenominator[1], call_amounts[1]) == SafeMath.mul(tokenIdAndNumeratorDenominator[2], call_amounts[0]),"anyERC1155ForERC20: wrong ratio");
		require(ArrayUtils.arrayEq(data, abi.encodeWithSignature("mintAndTransfer(address,address,uint256,uint256,string,bytes)", addresses[1], addresses[4], tokenIdAndNumeratorDenominator[0], call_amounts[0], tokenURI, signature)));
		...
		returns (uint)
	{
		...
		require(new_fill <= uints[1],"anyERC20ForERC1155: new fill exceeds maximum fill");
		require(SafeMath.mul(tokenIdAndNumeratorDenominator[1], call_amounts[0]) == SafeMath.mul(tokenIdAndNumeratorDenominator[2], call_amounts[1]),"anyERC20ForERC1155: wrong ratio");
		require(ArrayUtils.arrayEq(counterdata, abi.encodeWithSignature("mintAndTransfer(address,address,uint256,uint256,string,bytes)", addresses[4],addresses[1], tokenIdAndNumeratorDenominator[0], call_amounts[0], tokenURI, signature)));
